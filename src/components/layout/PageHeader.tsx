  ...
  return (
    <motion.header 
      ...
      className={`${variant === 'centered' ? 'flex-col items-center text-center' : 
        ...
        'flex-col sm:flex-row sm:justify-between sm:items-center'} mb-4 sm:mb-6 ${className}`}
    >
        ...
        <div className={`flex ${variant === 'compact' ? 'items-center gap-3' : 'flex-col'}`}>
          {variant === 'compact' ? (
            <div className="flex items-center gap-3">
              ...
              <div>
                <motion.h1 variants={itemVariants} className={`text-xl sm:text-2xl font-bold ${variant === 'compact' ? 'mb-0' : 'mb-1'}`}>
                  <EditableField
                    value={title}
      ...
      <div className="flex flex-col space-y-4 mt-4 sm:mt-0">
        {actions && (
          <motion.div 
            ...
            className={`flex flex-wrap items-center gap-2 ${variant === 'centered' ? 'justify-center' : 'justify-start sm:justify-end'}`}
          >
            {actions}
          </motion.div>
        )}
