      ...
      >
        <div className="relative flex items-center mb-6">
          {showLeftScroll && (
            ...
            <TabsList 
            ref={tabsListRef}
            className="mb-0 w-full justify-start overflow-x-auto scrollbar-none flex-nowrap bg-muted/50"
          ...
          >
            {tabs.map(tab => (
              <TabsTrigger 
                key={tab.value} 
                value={tab.value}
                className="transition-all duration-200 whitespace-nowrap text-xs sm:text-sm px-2 sm:px-4 py-1 sm:py-2 data-[state=active]:bg-primary/10 data-[state=active]:text-primary"
              >
                {tab.label}
              </TabsTrigger>
            ))}
