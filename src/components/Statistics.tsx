...
import EnhancedFinancialCharts from './statistics/EnhancedFinancialCharts';
...
const Statistics = () => {
    ...
    switch (currentChart) {
      ...
      case 'financial':
        return [
          ...
          { key: "profitability", header: "Rentabilité (FCFA)" },
  ...
  const handleExportData = async () => {
    ...
    link.href = url;
    link.download = `statistiques-${currentChart}-${new Date().toISOString().split('T')[0]}.json`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
    
    console.log(`Exportation des données ${currentChart} terminée`);
  };
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-3 md:p-6 mb-6 shadow-sm dark:bg-gray-800 dark:border-gray-700">
        ...
        <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-2 md:p-4 overflow-x-auto">
          <div className={`min-w-full ${isMobile ? 'min-w-[500px]' : ''}`}>
            ...
            {currentChart === 'financial' && <EnhancedFinancialCharts />}
