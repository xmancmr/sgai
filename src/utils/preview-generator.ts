...
export const generatePreviewHTML = (
  ...
  const { title, columns, locale } = options;
  ...
  const tableHeaders = (
    ...
    .map(
      (col) =>
        `<th class="px-4 py-2 bg-gray-100 border-b dark:bg-gray-700 dark:border-gray-600 dark:text-white">${col.header}</th>`
    )
    .join('');
  ...
  const tableRows = data
    .map((row) => {
      const cells = (
        columns || Object.keys(row).map((key) => ({ key, header: key }))
      )
        .map((col) => {
